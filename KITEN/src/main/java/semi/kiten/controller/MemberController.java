...
public class MemberController {
	...
	/*
	 * 가입하기 버튼 클릭 -> 요청 : /user/userInsert.do 뷰페이지 : user/userJoin_ok.jsp
	 */
	...
	public ModelAndView userInsert(MemberVO memberVO) {
		...
		if (result == 1)
			message = memberVO.getM_id() + " 님의 회원 가입이 완료되었습니다!";
	...
	public MemberVO idSelect(HttpSession session, Model m) {
		...
		MemberVO result = memberService.idSelect(userNo);
	...
	public String remove(@ModelAttribute MemberVO vo, Model model) {

		boolean result = memberService.passChk(vo.getM_id(), vo.getM_password()); // 패스워드 검사하는 메소드
